...
signal reg5,reg6,reg7: std_logic_vector(31 downto 0);
signal pc: std_logic_vector(31 downto 0);

begin
	reg_clkgenerator: not_gate port map(clk, reg_clk);
	ins1: Instruction port map(zero=>eqzero, clk=>clk, jump=>jump, branch=>branch, reset=>reset, instruction=>ins, pc0 => pc);
	dec: decode port map(ins=>ins, regS=>regS, regT=>regT, regD=>regD, imm=>imm, opcode=>opcode, func=>func, shamt=>shamt);
	reg: register_file port map(outA=>busA, outB=>busB, reg5 => reg5, reg6 => reg6, reg7 => reg7, input=>busW, writeEnable=>regWr, regASel=>regS, regBSel=>regT, writeRegSel=>regW, clock=>clk);
	...
	mem0: syncram generic map (mem_file=>"sort_corrected_branch.dat") port map (clk => reg_clk, cs => '1', oe => not_memWr, we => memWr, addr => ALUResult, din=>busB, dout=>memOut);
